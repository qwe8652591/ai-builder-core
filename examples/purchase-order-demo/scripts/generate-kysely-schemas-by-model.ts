...
`;
  ...
  if (enumsToImport.size > 0) {
    content += `import type { ${Array.from(enumsToImport).join(', ')} } from '../src/domain/${config.name}.model';\n`;
  }
...
`;
  ...
  modelConfigs.forEach(config => {
    content += `export * from './${config.name}.schema';\n`;
  });
  
  if (extendedModelNames.size > 0) {
    content += `\n// 扩展 Schema\n`;
    extendedModelNames.forEach(modelName => {
      content += `export * from './extensions/${modelName}.schema.ext';\n`;
    });
  }
  ...
  tablesBySource.forEach((tables, source) => {
    ...
    const importPath = source.includes('extensions/') 
      ? `./${source}`  // extensions/XXX.ext → ./extensions/XXX.ext
      : `./${source}.schema`;  // XXX → ./XXX.schema
    content += `import type {\n  ${types}\n} from '${importPath}';\n`;
  });
 ...
 * 生成运行态可用的表结构定义文件
 * 
 * 这个文件会被 kysely.browser.ts 导入，用于动态创建表结构
 */
function generateTableDefinitions(): string {
  const tables = metadataStore.getAllTables();
...
`;
  ...
  // 生成表定义数组
  content += `export const tableDefinitions: TableDefinition[] = [\n`;
  ...
  tables.forEach((table, index) => {
    ...
    table.columns.forEach((col) => {
      // 映射类型到 SQLite 类型
      let sqliteType: 'text' | 'real' | 'integer' | 'blob' = 'text';
      const typeStr = col.type.toLowerCase();
      
      if (typeStr.includes('number') || typeStr.includes('decimal') || typeStr.includes('columntype')) {
        sqliteType = 'real';
      } else if (typeStr.includes('boolean') || typeStr.includes('integer')) {
        sqliteType = 'integer';
      } else if (typeStr.includes('date')) {
        sqliteType = 'text'; // SQLite 使用 text 存储日期
      }
      
      const isPrimaryKey = col.name === 'id' || col.isGenerated;
      const isNotNull = !col.type.includes('null') && col.name !== 'id';
      const isForeignKey = col.isForeignKey;
      
      content += `      { name: '${col.name}', type: '${sqliteType}'`;
      
      if (col.name === 'id') {
        content += `, primaryKey: true`;
      }
      if (isNotNull && col.name !== 'id') {
        content += `, notNull: true`;
      }
      if (isForeignKey && col.sourceField) {
        // 推断引用的表
        const refTable = col.name.replace('_id', '') + 's';
        content += `, references: '${refTable}.id'`;
      }
      if (col.comment) {
        content += `, comment: '${col.comment.replace(/'/g, "\\'")}'`;
      }
...
async function main() {
  ...
  const generatedDir = path.join(projectRoot, 'generated');  // 📁 所有生成文件输出到 generated/
  ...
  modelConfigs.forEach(config => {
    ...
    const outputPath = path.join(generatedDir, `${config.name}.schema.ts`);
    
    fs.writeFileSync(outputPath, schemaContent, 'utf-8');
    console.log(`   ✅ generated/${config.name}.schema.ts`);
  ...
  const databasePath = path.join(generatedDir, 'database.schema.ts');
  fs.writeFileSync(databasePath, databaseContent, 'utf-8');
  console.log(`   ✅ generated/database.schema.ts\n`);

  // ========== 步骤 7: 生成运行态表定义文件 ==========
  console.log('📝 生成运行态表定义：\n');
  const tableDefsContent = generateTableDefinitions();
  const tableDefsPath = path.join(generatedDir, 'table-definitions.ts');
  fs.writeFileSync(tableDefsPath, tableDefsContent, 'utf-8');
  console.log(`   ✅ generated/table-definitions.ts\n`);
  ...
  modelConfigs.forEach(config => {
    console.log(`   └── ${config.name}.model.ts          (领域模型定义)`);
  });
  ...
  console.log('   └── table-definitions.ts              ← 运行态表定义\n');
